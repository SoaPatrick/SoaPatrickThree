...
function soapatrickthree_scripts() {
	...
	wp_enqueue_style( 'soapatrickthree-style', get_stylesheet_uri(), array('lity-css') );		
	...
	wp_enqueue_script( 'touchswipe', get_template_directory_uri() . '/js/touchswipe.min.js', array('jquery'), '', true );	
	
	wp_enqueue_script( 'lity', get_template_directory_uri() . '/lity/lity.min.js', array('jquery'), '', true );		
	...
	wp_enqueue_style( 'lity-css', get_template_directory_uri() . '/lity/lity.min.css' );							
...
if ( function_exists( 'add_image_size' ) ) { 
	...
	add_image_size( 'full-width', 750, 9999 ); //1140 pixels wide (and unlimited height)    
}
...
function posts_link_next_class($format){
     $format = str_replace('href=', 'class="btn btn-light-grey" href=', $format);
     return $format;
}
...
function set_tag_cloud_font_size($args) {
    $args['smallest'] = 14; /* Set the smallest size to 12px */
    $args['largest'] = 30;  /* set the largest size to 19px */
    return $args; 
}
...
/**
 * add lity attribute to links to media
 */
 

add_filter('the_content', 'add_data_lity_attribute');
function add_data_lity_attribute ($content) {
	global $post;	
	$pattern = "/<a(.*?)href=('|\")(.*?).(bmp|gif|jpeg|jpg|png)('|\")(.*?)>/i";
  	$replacement = '<a$1data-lity href=$2$3.$4$5$6</a>';
